...
- `x[, i] <- NULL` on a list environment matrix `x` with partial
  dimnames (some dimensions named, others not) would produce an error.

- `parse_env_subset(x[i, j])` would produce an error when the list
  environment `x` had named elements and a subscript was out of bounds.
...
- [`map()`](https://listenv.futureverse.org/reference/mapping.md) is
  formally defunct; use
  [`mapping()`](https://listenv.futureverse.org/reference/mapping.md)
  instead.
...
- Add [`aperm()`](https://rdrr.io/r/base/aperm.html) and
  [`t()`](https://rdrr.io/r/base/t.html) for list-environment arrays.

- [`parse_env_subset()`](https://listenv.futureverse.org/reference/parse_env_subset.md)
  gained argument `is_variable` to control whether or not the inferred
  element named should be checked if it is a valid variable name.
...
- [`map()`](https://listenv.futureverse.org/reference/mapping.md) is
  formally deprecated; use
  [`mapping()`](https://listenv.futureverse.org/reference/mapping.md)
  instead.

- Dropped the defunct usage of `listenv(length = n)`, meaning it no
  longer gives an error but instead produces a `listenv` with an element
  named `length`.
...
- `parse_env_subset(x[1, names])` on a listenv `x` matrix would throw
  ...
  if one of the elements in `names` specifies a non-existing column
  name.
...
- Now it is possible to set the dimension on an empty list environment
  without first resizing it with
...
- Added [`lengths()`](https://rdrr.io/r/base/lengths.html) for list
  environments. Requires R (\>= 3.3.0).

- `dim_na(x) <- dims`, where `dims` contain exactly one missing value,
  will set the “missing” dimension based on the length of `x` and the
  other dimensions specified, e.g. with `length(x) == 6`,
  `dim_na(x) <- c(2, NA)` will set `dim(x) <- c(2, 3)`. This works for
  all types of object to which dimensions can be assigned - not only
  list environments.

- Added [`is.matrix()`](https://rdrr.io/r/base/matrix.html),
  [`is.array()`](https://rdrr.io/r/base/array.html),
  [`as.vector()`](https://rdrr.io/r/base/vector.html), and
  [`as.matrix()`](https://rdrr.io/r/base/matrix.html) for list
  environments.

### Bug Fixes

- [`print()`](https://rdrr.io/r/base/print.html) on a named, empty list
  environment would output an empty string.
...
- Function
  [`map()`](https://listenv.futureverse.org/reference/mapping.md) has
  been renamed to
  [`mapping()`](https://listenv.futureverse.org/reference/mapping.md)
  and same for the corresponding replacement function. The
  [`map()`](https://listenv.futureverse.org/reference/mapping.md) and
  `map<-()` functions will soon be deprecated and eventually defunct.
...
- `parse_env_subset(x[[idx]])` for list environment `x` and index `idx`
  claimed `x[[idx]]` existed as long as `idx` in `[1,length(x)]`, but it
  forgot to check if element really existed, which may not be true if
  `x` has been expanded.
...
- Added [`unlist()`](https://rdrr.io/r/base/unlist.html) and
  [`all.equal()`](https://rdrr.io/r/base/all.equal.html) for list
  environments.

### Deprecated and Defunct

- Deprecated `x <- listenv(length = n)`; use
  `x <- listenv(); length(x) <- n` instead.
...
- Added
  [`as.listenv()`](https://listenv.futureverse.org/reference/listenv.md).
...
- [`print()`](https://rdrr.io/r/base/print.html) on `listenv` handles
  empty and no-named `listenv`:s better.
...
- [`get_variable()`](https://listenv.futureverse.org/reference/get_variable.md)
...
- Added [`print()`](https://rdrr.io/r/base/print.html) for `listenv`:s.
